...
- Phase 1, prototype: laser-cut birch plywood, stacked layers gluing into a cavity for the
  Pi. Cheap, fast iteration via online services (Damengo, L'Atelier de Decoupe Numerique).
...
The local loop, end to end, on the existing Pi (192.168.1.119), with one dummy widget (the
...
192.168.1.119, and the server-in-Docker path. Pairing (QR/WiFi) is Phase 1/2, tracked in
issue #1.
